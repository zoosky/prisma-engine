...
use connector_interface::{self as iface, result_ast::*, write_ast::*, UnmanagedDatabaseWriter};
    ...
    T: Transactional + SqlCapabilities + Send + Sync + 'static,
{
    fn execute(
        &self,
        db_name: String,
        write_query: RootWriteQuery,
    ) -> iface::IO<WriteQueryResult> {
        ...
        iface::IO::new(async move { Ok(fut.await?) })
    }

    fn execute_raw(&self, db_name: String, query: String) -> iface::IO<Value> {
        let fut = async move {
            let conn = self.executor.get_connection(&db_name).await?;
