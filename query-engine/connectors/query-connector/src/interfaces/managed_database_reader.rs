...
pub trait ManagedDatabaseReader {
    ...
    fn get_many_records<'a>(
        ...
        selected_fields: &'a SelectedFields,
    ) -> BoxFuture<'a, crate::Result<ManyRecords>>;
    ...
    ) -> BoxFuture<'a, crate::Result<Vec<ScalarListValues>>>;

    /// Count the items in the model with the given arguments.
    fn count_by_model<'a>(&'a self, model: ModelRef, query_arguments: QueryArguments) -> BoxFuture<'a, crate::Result<usize>>;

    /// Count the items in the table.
    fn count_by_table<'a>(&'a self, database: &'a str, table: &'a str) -> BoxFuture<'a, crate::Result<usize>>;
}
